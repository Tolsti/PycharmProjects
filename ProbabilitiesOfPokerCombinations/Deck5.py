...
deck = [(f, s) for s in '♥♣♦♠' for f in range(2, 15)]


def face_combination(hand):
    faces, suits = sorted([f[0] for f in hand]), [s[1] for s in hand]
    # faces = sorted([i for h in hand for i, s in enumerate('234567891JQKA', start = 2) if h[0] == s])
    repeat_face = [f for f in faces if faces.count(f) in range(2, 5)]
    ...
    straight = (faces[-1] - faces[0] == 4 or faces == [2, 3, 4, 5, 14]) and len(repeat_face) == 0
    flush = len([s for s in suits if suits.count(s) == 5]) == 5
    
    if faces == [10, 11, 12, 13, 14] and flush:
        return 'ROYAL FLUSH'
    ...
    if straight:
        return 'STRAIGHT'
    if 13 in faces and 14 in faces:
        return 'ACE KING'
    return 'NOTHING'
...
def take_five_random_cards(t_deck, count_cards=5):
    tmp_deck, five_cards = t_deck.copy(), list()
